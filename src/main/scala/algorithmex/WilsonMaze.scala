...
object WilsonMaze extends MazeGenerator {
  ...
  override def generate[CC <: C](random: Random, maze: Maze[CC]): Maze[CC] = {
    ...
    @tailrec
    def roaming(maze: Maze[CC], unvisited: Set[Position2D],
                pos: Position2D, prev: List[Position2D],
      ...
      if (unvisited.contains(pos) && neighbors.nonEmpty) {
        ...
        while (set.contains(p)) {
          path match {
            case ::(head, next) => {
              path = next; set = set - head
            }
            case Nil =>
          }
        }
        path = p +: path; set = set + p
        roaming(maze, unvisited, p, path, set)
      } else prev
    }
    @tailrec
    def loop(maze: Maze[CC], unvisited: Set[Position2D]): Maze[CC] = {
      if (unvisited.nonEmpty) {
        ...
        val ret = roaming(maze, unvisited, pos, path, pathSet)
        var newMaze = maze
