...
//      Thread.sleep(2000)
  ...
  test("Braid") {
    val mazes = List(PolarMaze(15), WeaveMaze(20, 20))
    mazes.foreach {
      m => {
        val maze = RecurBackTrackMaze.generate(rand, m)
        ...
        println(s"${maze.info.name} dead end count: old=${oldDeadEnds.size}, new=${newDeadEnds.size}")
